...
function Dots() {
      ...
      return <span>{".".repeat(d)}<span style={{opacity:0}}>{".".repeat(3-d)}</span>span></span>span>;
}
...
function ProcessingOverlay({ message, sub }) {
      ...
      return (
              <div style={{ background: "#fff", borderRadius: 12, padding: 28, textAlign: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.10)" }}>
                    <div style={{ fontSize: 40, marginBottom: 12 }}>Loading</div>div>
                    <div style={{ fontSize: 16, fontWeight: 800, color: green, marginBottom: 6 }}>{message}<Dots /></div>div>
                  {sub && <div style={{ fontSize: 12, color: "#888", marginBottom: 16 }}>{sub}</div>div>}
                    <div style={{ fontSize: 13, color: "#aaa" }}>経過時間: {elapsed}秒</div>div>
      ...
      return (
              <div style={{ fontFamily: "'Hiragino Sans','Noto Sans JP',sans-serif", background: "#f7f5f0", minHeight: "100vh", maxWidth: 600, margin: "0 auto" }}>
                    <div style={{ background: green, color: "#fff", padding: "16px 16px 12px" }}>
                            <div style={{ fontSize: 10, letterSpacing: 2, opacity: 0.6 }}>本せどり ハレ</div>div>
                            <div style={{ fontSize: 20, fontWeight: 800, marginTop: 2 }}>本棚スキャナー</div>div>
                            <div style={{ display: "flex", gap: 4, marginTop: 12 }}>
                              ...
                              <div key={i} style={{ flex: 1, textAlign: "center" }}>
                                            ...
                                            <div style={{ fontSize: 8, marginTop: 3, opacity: i <= step ? 1 : 0.4, lineHeight: 1.2 }}>{s}</div>div>
                              </div>div>
                            ))}
                            </div>div>
                    </div>div>
                    ...
                    <div style={{ padding: 16 }}>
                            <div style={{ background: "#fff", borderRadius: 10, padding: 14, marginBottom: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
                                      <div style={{ fontSize: 12, fontWeight: 700, color: green, marginBottom: 6 }}>
                                                  Keepa APIキー
                                          {keySaved && <span style={{ marginLeft: 8, background: lightGreen, color: green, fontSize: 10, padding: "2px 6px", borderRadius: 4 }}>保存済み</span>span>}
                                      </div>div>
                                      <div style={{ display: "flex", gap: 8 }}>
                                          ...
                                          {!keySaved && keepaKey && (
                                <button onClick={saveKey} style={{ background: green, color: "#fff", border: "none", borderRadius: 7, padding: "0 10px", cursor: "pointer", fontSize: 12, fontWeight: 700 }}>保存</button>button>
                                                  )}
                                          {keySaved && (
                                <button onClick={clearKey} style={{ background: "#fee", color: red, border: "none", borderRadius: 7, padding: "0 10px", cursor: "pointer", fontSize: 12 }}>削除</button>button>
                                                  )}
                                      </div>div>
                                {!keepaKey && <div style={{ fontSize: 11, color: "#999", marginTop: 4 }}>未入力の場合はISBN取得まで行います</div>div>}
                            </div>div>
                                ...
                                {!processingImg ? (
                                              <>
                                                              <div onClick={() => fileRef.current.click()}
                                                                                ...
                                                                                <div style={{ fontSize: 40 }}>写真</div>div>
                        ...
                        {step === 1 && (
                            <div>
                                        <div style={{ background: "#fff", borderRadius: 10, padding: 14, marginBottom: 14, boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
                                                      ...
                                                      <div style={{ fontSize: 12, color: "#666", marginBottom: 10 }}>タップして編集・削除できます</div>div>
                                            {titles.map((t, i) => (
                                                <div key={i} style={{ display: "flex", gap: 8, marginBottom: 6, alignItems: "center" }}>
                                                                  ...
                                                                  <button onClick={() => setTitles(titles.filter((_, j) => j !== i))}
                                                                                          style={{ background: "#fee", color: red, border: "none", borderRadius: 6, padding: "6px 10px", cursor: "pointer", fontWeight: 700 }}>×</button>button>
                                                </div>div>
                                              ))}
                        ...
                        {step === 2 && (
                            <div style={{ background: "#fff", borderRadius: 12, padding: 24, textAlign: "center", boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
                                        <div style={{ fontSize: 36, marginBottom: 12 }}>検索中</div>div>
                                        <div style={{ fontSize: 15, fontWeight: 700, color: green }}>国会図書館でISBN検索中<Dots /></div>div>
                                        ...
                                        <div style={{ fontSize: 14, fontWeight: 700, color: green, marginBottom: 4 }}>{ndlProgress}%</div>div>
                                        <div style={{ fontSize: 12, color: "#888", marginTop: 4 }}>
                                            {ndlCurrent && `「${ndlCurrent}」を検索中`}
                                        </div>div>
                                        <div style={{ fontSize: 11, color: "#bbb", marginTop: 8 }}>{titles.length}冊中 {Math.round(titles.length * ndlProgress / 100)}冊完了</div>div>
                            </div>div>
                            )}
                    
                        {step === 3 && (
                            <div style={{ background: "#fff", borderRadius: 12, padding: 24, textAlign: "center", boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
                                        <div style={{ fontSize: 36, marginBottom: 12 }}>判定中</div>div>
                                        <div style={{ fontSize: 15, fontWeight: 700, color: green }}>Keepaで利益判定中<Dots /></div>div>
                                        <div style={{ margin: "16px 0 4px", background: "#eee", borderRadius: 4, height: 10 }}>
                                                      <div style={{ width: `${keepaProgress}%`, background: yellow, height: 10, borderRadius: 4, transition: "width 0.3s" }} />
                                        </div>div>
                                        <div style={{ fontSize: 14, fontWeight: 700, color: "#b07a00", marginBottom: 4 }}>{keepaProgress}%</div>div>
                                        <div style={{ fontSize: 11, color: "#bbb", marginTop: 8 }}>判定基準: ランク{RANK_THRESHOLD.toLocaleString()}位以内・利益{PROFIT_THRESHOLD}円以上</div>div>
                            </div>div>
                            )}
                            ...
                            <div>
                                        <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
                                                      <div style={{ flex: 1, background: lightGreen, borderRadius: 10, padding: "12px 10px", textAlign: "center" }}>
                                                                      <div style={{ fontSize: 28, fontWeight: 800, color: green }}>{profitBooks.length}</div>div>
                                                      </></></span>
                                  <div style={{ fontSize: 11, color: green, fontWeight: 700 }}>利益本</div>div>
              </div>div>
                        <div style={{ flex: 1, background: "#fff", borderRadius: 10, padding: "12px 10px", textAlign: "center", boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
                                            <div style={{ fontSize: 28, fontWeight: 800, color: "#888" }}>{otherBooks.length}</div>div>
                                            <div style={{ fontSize: 11, color: "#888", fontWeight: 700 }}>その他</div>div>
                        </div>div>
                        <div style={{ flex: 1, background: "#fff", borderRadius: 10, padding: "12px 10px", textAlign: "center", boxShadow: "0 1px 4px rgba(0,0,0,0.07)" }}>
                                            <div style={{ fontSize: 24, fontWeight: 800, color: green }}>¥{profitBooks.reduce((s, b) => s + (b.keepa?.profit ?? 0), 0).toLocaleString()}</div>div>
                                            <div style={{ fontSize: 11, color: "#888", fontWeight: 700 }}>見込み利益</div>div>
                        </div>div>
          </div>
                          <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
                                            <button onClick={() => exportCSV(profitBooks, "利益本リスト.csv")}
                                                                style={{ flex: 1, background: green, color: "#fff", border: "none", borderRadius: 8, padding: "10px 8px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>利益本CSV</button>button>
                                            <button onClick={() => exportCSV(otherBooks, "その他リスト.csv")}
                                                                style={{ flex: 1, background: "#fff", color: "#555", border: "2px solid #ddd", borderRadius: 8, padding: "10px 8px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>その他CSV</button>button>
                                            <button onClick={() => { setStep(0); setImages([]); setTitles([]); setBooks([]); }}
                                                                style={{ flex: 1, background: lightGreen, color: green, border: "none", borderRadius: 8, padding: "10px 8px", fontSize: 13, fontWeight: 700, cursor: "pointer" }}>最初から</button>button>
                          </div>div>
              ...
              {(activeTab === "profit" ? profitBooks : otherBooks).map((b, i) => {
                            ...
                            return (
                                                <div key={i} style={{ background: "#fff", borderRadius: 10, marginBottom: 10, padding: "12px 14px", borderLeft: `4px solid ${isProfit ? green : "#ddd"}`, boxShadow: "0 1px 4px rgba(0,0,0,0.06)" }}>
                                                                      <div style={{ fontSize: 14, fontWeight: 700, color: "#1a1a1a", marginBottom: 4 }}>{b.title}</div>div>
                                                    {b.ndlTitle && b.ndlTitle !== b.title && <div style={{ fontSize: 11, color: "#888", marginBottom: 4 }}>{b.ndlTitle}</div>div>}
                                                                      <div style={{ fontSize: 12, color: "#555", lineHeight: 1.8 }}>
                                                                          {b.author && <span style={{ marginRight: 12 }}>{b.author}</span>span>}
                                                                          {b.publisher && <span>{b.publisher}</span>span>}
                                                                      </div>div>
                                                    {b.isbn && <div style={{ fontSize: 11, color: "#aaa", fontFamily: "monospace", marginTop: 2 }}>ISBN: {b.isbn}</div>div>}
                                                    {k && (
                                                    ...
                                                    {!b.isbn && <div style={{ fontSize: 11, color: "#f0a500", marginTop: 4 }}>ISBNが見つかりませんでした</div>div>}
